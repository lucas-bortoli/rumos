...
type TermId = number & { _tag?: "termId" };
...
type QuestionId = number & { _tag?: "questionId" };
...
type BattleState =
  | { kind: "InitialDialog" }
  | { kind: "UserTurn"; question: Question; choices: Term[] }
  | { kind: "OpponentTurn" }
  | { kind: "GameOver" }
  | { kind: "Victory" };
...
export default class Battle implements ImperativeObject {
  ...
  constructor() {
      ...
      questions: new Map(
        [
          ...
          },
          {
            id: 19,
            dialogue:
              "Dois fiadores onde qualquer um pode ser cobrado - isso é responsabilidade...?",
    ...
    this.opponent.questions.forEach((question) => {
      if (!this.opponent.terms.has(question.correctResponseTerm)) {
        console.error("Questão tem uma resposta correta que não existe:", { question });
  ...
  private pickQuestion() {
    const allTerms = [...this.opponent.terms.values()];
    const allQuestions = [...this.opponent.questions.values()];
    const pickedQuestion = allQuestions[Math.floor(Math.random() * allQuestions.length)];
    const correctChoice = this.opponent.terms.get(pickedQuestion.correctResponseTerm)!; // null validado no construtor
    const wrongChoices = pickRandomUniqueChoices(
      allTerms.filter((term) => term.id !== correctChoice.id),
      4
    );

    const question = pickedQuestion;
    const choices = shuffleArray([...wrongChoices, correctChoice]);
