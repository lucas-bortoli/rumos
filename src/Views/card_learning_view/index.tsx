...
import { CardLearningWindow } from "./_windows";
import Img1 from "./trilha02.png";

interface CardLearningIntroProps {
  trail: KnowledgeTrail;
}

export default function CardLearningIntro(props: CardLearningIntroProps) {
  const windowing = useWindowing();
  const currentWindowKey = useCurrentWindowKey();

  function start() {
    windowing.createWindow(CardLearningWindow, { trail: props.trail });
  ...
  return (
    <main className="relative h-full w-full overflow-x-hidden overflow-y-scroll bg-white pb-16">
      ...
      <section className="flex flex-col items-center gap-4 text-center">
        ...
        <p className="mx-8">Aprenda o conteúdo através de cartas.</p>
        <div className="mx-8 text-left">
          <p className="font-bold">Como funciona?</p>
          <ul className="ml-8 list-disc">
            <li>Cada carta tem uma pergunta de um lado e uma resposta do outro;</li>
            <li>O objetivo é testar o quanto você sabe e ajudá-lo a memorizar as informações;</li>
            <li>Após responder cada carta, você poderá ver a resposta correta.</li>
          </ul>
        </div>
